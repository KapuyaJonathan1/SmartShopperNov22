...
function Info({ health, budget, image, uid, store, switchPage }) {
...
Do not add any extra text, introductions, or conclusions. Separate the sections with a header for advantages and disadvantages.`;
  ...
  async function getInfo() {
    ...
    try {
      ...
      const response1 = await axios.post(
        "https://smartshoppernov22.onrender.com/api/get-info",
        {
          prompt: healthPrompt,
          uid: uid, // Include UID
          delete: false, // Do not delete yet
        }
      );
      ...
      // Third Prompt: Alternatives (Delete Image After Processing)
      const response3 = await axios.post(
        "https://smartshoppernov22.onrender.com/api/get-info",
        {
          prompt: alternativesPrompt,
          uid: uid,
          delete: true, // Delete the image after this step
        }
      );
    ...
    <div className="container d-flex flex-column gradient-bg" style={{ height: "100vh" }}>
      ...
      <div className="d-flex justify-content-center align-items-center" style={{ marginTop: "auto", padding: "20px" }}>
        <button
          ...
          disabled={loading}
          style={{ display: `${information[2] ? "none" : "inline-block"}` }}
        >
